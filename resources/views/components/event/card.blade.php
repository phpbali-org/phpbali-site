<div class="rounded-lg overflow-hidden border border-gray-400 p-6 m-4">
    ...
    @if (auth()->check() && (auth()->user()->isStaff() || auth()->user()->isAdmin()))
        <div class="flex justify-end mt-4 ml-auto">
            <button type="button" data-name="{{ $event->name }}" data-href="{{ $event->path() }}" class="mr-4 delete__event bg-red-600 hover:bg-red-500 text-white font-semibold py-2 px-4 border border-red-600 rounded shadow">Hapus</button>
            @if ($event->published)
