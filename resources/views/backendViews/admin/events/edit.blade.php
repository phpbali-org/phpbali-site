    ...
    <div class="col-xs-12" id="workshop-form">
        <div class="white-box">
            <form class="form-horizontal form-material" method="POST" enctype="multipart/form-data" action="{{ route('admin.event.update', ['slug' => $event->slug]) }}">
                ...
                <div class="form-group">
                    <label class="col-xs-12">Start Datetime</label>
                    <div class="col-xs-12">
                        <input type="text" name="start_datetime" id="startDatetime" class="form-control form-control-line no-block dateTime" placeholder="dd-mm-yyyy HH:mm" value="{{ $start_datetime }}" />
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-xs-12">End Datetime</label>
                    <div class="col-xs-12">
                        <input type="text" name="end_datetime" id="endDatetime" class="form-control form-control-line no-block dateTime" placeholder="DD-MM-YYYY HH:mm" value="{{ $end_datetime }}" />
                    </div>
                </div>
                <div class="form-group">
...
<script src="{{ asset('js/plugins/inputmask/inputmask.dependencyLib.min.js') }}"></script>
<script src="{{ asset('js/plugins/inputmask/inputmask.min.js') }}"></script>
<script src="{{ asset('js/plugins/inputmask/inputmask.date.extensions.min.js') }}"></script>
<script src="{{ asset('js/bootstrap-toggle.min.js') }}"></script>
<script>
    const $dateTimeMask = document.getElementsByClassName('dateTime');
    const dateTimeMask = new Inputmask("datetime", {
        mask: "1-2-y h:s",
    ...
    let i;
    if ($dateTimeMask) {
        for (i = 0; i < $dateTimeMask.length; i++) {
            dateTimeMask.mask($dateTimeMask);
        }
    }
